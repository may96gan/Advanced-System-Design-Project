...
def run_server(host='127.0.0.1', port=8080, database_url='mongodb://localhost:27017/'):
    ...
    def getUser(user_id):
        ...
        user['birthday'] = datetime.datetime.fromtimestamp(u['birthday']/1000)
        g = user['gender']
        if g == 0:
            user['gender'] = 'male'
        elif g == 1:
    ...
    def getUserSnapshots(user_id):
        ...
        for s in snapshots:
            s['datetime'] = datetime.datetime.fromtimestamp(s['datetime']/1000)
    ...
    def getUserSnapshot(user_id, snapshot_id):
        ...
        if 'feelings' in snapshot:
            res.append('feelings')
        return flask.render_template('snapshot.html',user=user,id=snapshot['_id'], datetime=datetime.datetime.fromtimestamp(snapshot['datetime']/1000),results=res)
